(function ($) {
    ...
    DayScheduleSelector.prototype.renderDays = function () {
        var stringDays = this.options.stringDays
            , days = this.options.days
            , html = '';
        ...
        $.each(days, function (i, _) {
            html += `
        <div class="schedule-days" data-d=${_}>${stringDays[i] || ''}</div>
      `;
        });
        this.$el.find('.day-schedule-selector').html(html);
    };
    ...
    DayScheduleSelector.prototype.renderTime = function (isSingle) {
        var start = this.options.startTime
            , end = this.options.endTime
            , interval = this.options.interval
            , days = this.options.days
            , $el = this.$el.find('.schedule-days');

        var timeArr = generateDates(start, end, interval);

        $el.each(function(index, el) {
            for (var i = 0; i < days.length; i++) {
                var thisDay = days[i];
                if ($(el).data('d') == thisDay) {
                    var $this = $(this);
                    $this.append($("<ul>", {"class": "schedule-days-time", "data-dd": thisDay}));

                    $.each(timeArr, function (i, d) {
                        var html = `<li class="time-slot" data-time="${hhmm(d)}" data-day="${thisDay}">${hhmm(d)}</li>`;
                        $this.find('.schedule-days-time').append(html);
                    });
                }
            }
        });
